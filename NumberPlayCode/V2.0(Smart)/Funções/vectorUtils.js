...
export function valuesSum(vector){
    let sum = doReduce(vector,(accumulated, currentValue) => accumulated + currentValue, 0)

    return sum;
}

export function addVector(originalVector, newVector){
    for (let number of newVector){
        originalVector.push(number);
    }
...
/**
 * 
 * @param {*} vector Lista de valores que serão modificados
 * @param {*} work Função que será aplicada a cada valor
 * @returns Array com os valores mapeados
 */
...
export const doReduce = (vector, work, initialValue) => {
  let accumulated = initialValue;

  for (let i = 0; i < vector.length; i++){
    accumulated = work(accumulated, vector[i]);
  }
